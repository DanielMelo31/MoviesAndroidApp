...
class UpcomingConcatAdapter(private val movieAdapter: MovieAdapter) : RecyclerView.Adapter<BaseConcatHolder<*>>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseConcatHolder<*> {
        val itemBinding = UpcomingMoviesRowBinding
    ...
    private inner class ConcatViewHolder(val itemBinding: UpcomingMoviesRowBinding) :
        BaseConcatHolder<MovieAdapter>(itemBinding.root) {
        override fun bind(adapter: MovieAdapter) {
            itemBinding.rvUpcomingMoviesRow.adapter = adapter
        }
    }
}